...
import {BannerVo} from "../data";
import TextArea from "antd/es/input/TextArea";

interface UpdateBannerFormProps {
    open: boolean;
    onCreate: (values: BannerVo) => void;
    onCancel: () => void;
    bannerVo: BannerVo;
}

const UpdateBannerForm: React.FC<UpdateBannerFormProps> = ({open, onCreate, onCancel, bannerVo}) => {
    const [form] = Form.useForm();
    const FormItem = Form.Item;

    useEffect(() => {
        if (bannerVo) {
            form.setFieldsValue(bannerVo);
        }
    }, [open]);
